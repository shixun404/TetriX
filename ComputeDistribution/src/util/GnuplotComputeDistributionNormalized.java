 ...
 *
 */
public class GnuplotComputeDistributionNormalized {
...
//			it.next(); //filter out the field line
		...
		if(!(minValue==-1 && maxValue==-1))
			...
			GnuplotDistribution.genDistribution(rawData, resultList, minValue, maxValue, distributionIntervals);
	
		return resultList;
	}
	
	public static List<String>[] normalization(double max, List<String>[] inputList)
	{
		List<String>[] resultList = new ArrayList[inputList.length];
		for(int i = 0;i<inputList.length;i++)
		{
			Iterator<String> iter = inputList[i].iterator();
			...
			while(iter.hasNext())
			{
				...
				double v = Double.parseDouble(s[0])/max;
				nl.append(v);
				...
				outputList.add(nl.toString());				
			}
			resultList[i] = outputList;
		}
		return resultList;
	...
	public static void generateDistributionGNUPlotFile(String rawDataFile, int fieldIndex, int distributionIntervals, String outputDir)
	{
		...
		// //normalization
		 List<String>[] resultList2 = normalization(maxValue, resultList);
		 ...
		 for(int i = 0;i<fIndex.length;i++)
		 {
			 ...
			 PVFile.print2File(resultList2[i], outputFile);
		 }
	}	
	...
	public static void main(String[] args)
	{
		if(args.length<5)
		{
			System.out.println("Usage: java GnuplotComputeDistribution [rawDataFile] [outputDir] [distributionIntervals] [extension] [minValue] [maxValue] [fieldIndexList]");
		...
		minValue = Double.parseDouble(args[4]);
		maxValue = Double.parseDouble(args[5]);
		int[] fieldID = new int[args.length-6];
		for(int i = 0;i<fieldID.length;i++)
			fieldID[i] = Integer.parseInt(args[i+6]);
