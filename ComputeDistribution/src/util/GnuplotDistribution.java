...
public class GnuplotDistribution {
	...
	public static void genDistribution(double[][] rawData, List<String>[] result, double min, double max, int scale)
	{
		int m = rawData.length;
		int distrIntervals = scale;
		...
		for(int i =0;i<m;i++)
		{
			...
			while(iter.hasNext())
			{
				...
				if(value>=upperBound)
					continue;
				int index = (int)((value-min)/unit);
				if(index == distrIntervals)
					index --;
				distResult[index]++;
			}
			ConversionHandler.convertArray2List4Comp(distResult, result[i], min, unit);
		}
	}
