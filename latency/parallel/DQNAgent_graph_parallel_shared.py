...
class DQNNetwork(nn.Module):
    def __init__(self, feature_dim, output_dim, T=2, N=20,
                    device=torch.device('cuda:0'), latency_mean=100, latency_std=15):
    ...
    def forward(self, state, start_id):
        ...
        adj_fc_ = adj_fc
        adj_fc = (adj_fc - self.latency_mean) / self.latency_std
        
        adj = (adj - self.latency_mean) / self.latency_std
...
class SharedParallelDQNAgent:
    ...
    def _update_shared_network(self, batch_size):
        """Update the shared Q-network with numerical stability"""
        ...
        dones = torch.tensor(dones, dtype=torch.float32, device=self.device)  # Changed to float32
        ...
        # Check for NaN in current Q-values
        if torch.isnan(current_q).any():
            print("Warning: NaN detected in current Q-values, skipping this batch")
            return 0.0
