...
class OptimizedGraphEnv(gym.Env):
    ...
    def __init__(self, num_nodes=100, K=8, num_sources=1, M=1, alpha=0.9, alpha_schedule=None, 
                 weight_mu=100, weight_sigma=15, weight_scale=1.0):
        ...
        self.diameter_mean = self.weight_mu * math.log(self.num_nodes) / math.log(self.K - 1)
        self.diameter_std = (self.weight_sigma * math.sqrt(math.log(self.num_nodes) / math.log(self.K - 1)) + 1e-5)
    ...
    def step(self, action):
        ...
        if not self.if_test:
            for p in range(self.M):
                ...
                # Combine rewards
                individual_reward = ((self.alpha * R_G + (1 - self.alpha) * R_M_p) - self.diameter_mean) / self.diameter_std - self.weight_scale * (weight_sum[p] - self.weight_mu) / self.weight_sigma
